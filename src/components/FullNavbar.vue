<template>
  <div class="backdrop fixed inset-0 z-30" @click.self="closeNavbar">
    <div class="absolute inset-8 shadow-2xl shadow-black grid duration-200" :class="{'lg:left-1/2': !activeSubMenuProduits }">
      ...
      <div class="bg-white flex justify-center">
        ...
        <transition name="scaleX">
          <div v-if="activeSubMenuProduits" class="relative bg-white text-black w-full flex flex-col justify-center items-center gap-4 col-span-3 duration-300">
            ...
            <router-link @click="closeNavbar" class="text-4xl text-center min-w-full bold duration-200 hover:-translate-x-2" to="/tailored">Vetement sur mesure</router-link>
            <router-link v-for="category in categorys" v-bind:key="category" @click="closeNavbar" class="text-4xl text-center min-w-full bold duration-200 hover:-translate-x-2" :to="{ name: 'ProductDetails', params: { name: category.name }}">{{ category.name }}</router-link>
          </div>
        </transition>
      <button @click="closeNavbar" class="close_button absolute h-14 w-14 top-4 right-4 duration-200 hover:rotate-45">
        <span class="h-1 w-3/4 block rotate-45 absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 bg-black"></span>
        <span class="h-1 w-3/4 block -rotate-45 absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 bg-black"></span>
      </button>
      </div>
    </div>
